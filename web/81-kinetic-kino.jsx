export default function T81KineticKino() {
  ...
  const specimens = [
    ...
    { id: "SP_05", title: "Th", v: 4, meta: "SLANT PRO · 60pt · LIGHT", w: "w-72", bg: "bg-background", textColor: "text-primary", chip1: "CORNICE", chip1Cls: "bg-magenta text-background", tagCls: "bg-primary text-background", grad: "from-background/85 via-background/30 to-transparent", dir: "t", italic: true, extraImg: "opacity-85", srcOverride: "https://images.unsplash.com/photo-1493663284031-b7e3aefcae8e?w=900&q=85&auto=format&fit=crop" },
    { id: "SP_06", title: "Gg", v: 2, meta: "GROTESK NEUE · 180pt", w: "w-80", bg: "bg-magenta", textColor: "text-background", chip1: "DRAFT", chip1Cls: "bg-primary text-background", tagCls: "bg-background text-magenta", grad: "from-magenta/30 via-transparent to-primary/60", dir: "r", italic: false, extraImg: "mix-blend-multiply opacity-85", srcOverride: "https://images.unsplash.com/photo-1481349518771-20055b2a7b24?w=1000&q=85&auto=format&fit=crop" },
  ];

  const inUseRows = [
    { side: "left", num: "USE 01 — EDITORIAL", title: "A weekly tabloid in two weights.", titleV: 2, body: "Sundown Gazette runs Grotesk Neue at axis 200 for body and axis 900 for sirens. The same file. The whole specimen on a 96-page issue: 80 KB.", img: "https://images.unsplash.com/photo-1531259683007-016a7b628fc3?w=1400&q=85&auto=format&fit=crop", chip: "USE · 01", pill: "SUNDOWN GAZETTE", grad: "from-primary/55 via-transparent to-transparent", dir: "t", stats: [{ v: "52", l: "Issues/yr" }, { v: "2", l: "Axes used" }, { v: "80kb", l: "Payload" }] },
                ...
                {[...specimens, ...specimens].map((s, i) => {
                  ...
                  return (
                    <figure key={`sp-${i}`} aria-hidden={i >= specimens.length ? "true" : undefined} className={`shrink-0 ${s.w} h-96 relative ${s.bg} border-2 border-background overflow-hidden`}>
                      <img alt={i < specimens.length ? `Specimen ${s.id}` : ""} className={`absolute inset-0 w-full h-full object-cover grayscale ${s.extraImg}`} src={s.srcOverride || "https://images.unsplash.com/photo-1542038784456-1ea8e935640e?w=900&q=85&auto=format&fit=crop"} />
                  ...
                  const figure = (
                    <figure className={`md:col-span-7 relative${r.side === "right" ? " md:order-2 order-1" : ""}`}>
                      ...
                      <div className="relative aspect-[16/10] overflow-hidden border-2 border-primary">
                        <img alt={`In Use 0${i + 1}`} className="absolute inset-0 w-full h-full object-cover grayscale" src={r.img} />
