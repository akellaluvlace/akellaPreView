// Ad-hoc smoke test for `lib/ast/operations/resize.ts applyResize`. Not a test
// harness — fire `node scripts/bench-resize.mjs` from repo root when you've
// touched the resize operation engine and want to verify the byte-overwrite
// produces clean source on the cases the (4b) gesture path actually hits.
//
// Each case logs PASS / FAIL — eyeballing the diff is the verification.
//
// Per CLAUDE.md "no test harnesses unless asked" this is a one-off
// developer-introspection script, not a CI gate.

import { parse } from "@babel/parser";
import MagicString from "magic-string";

// Inline a minimal port of applyResize so the script runs without a TS
// build step. Mirrors lib/ast/operations/resize.ts exactly — keep them in
// sync if the operation engine evolves. (The real module is what production
// uses; this is just for stand-alone smoke checking.)
...
function applyResize(source, op) {
  ...
  if (op.width !== undefined) desired.set("width", `'${op.width}'`);
  if (op.height !== undefined) desired.set("height", `'${op.height}'`);
...
const cases = [
  {
    name: "no style attr — insert one",
    source: `export default function T() { return <div data-dropin-id="aaaaaaaa">x</div>; }`,
    op: { oid: "aaaaaaaa", width: "248px", height: "120px" },
    expectChange: true,
  },
  {
    name: "existing style object — overwrite width",
    source: `export default function T() { return <div data-dropin-id="bbbbbbbb" style={{ width: '100px' }}>x</div>; }`,
    op: { oid: "bbbbbbbb", width: "248px" },
    expectChange: true,
  },
  {
    name: "existing style object — overwrite + append",
    source: `export default function T() { return <div data-dropin-id="cccccccc" style={{ width: '100px', color: 'red' }}>x</div>; }`,
    op: { oid: "cccccccc", width: "248px", height: "120px" },
    expectChange: true,
  },
  {
    name: "existing style with cn() expression — bail",
    source: `export default function T() { return <div data-dropin-id="dddddddd" style={someStyle()}>x</div>; }`,
    op: { oid: "dddddddd", width: "248px" },
    expectChange: false,
    expectReason: /CallExpression/,
  },
  {
    name: "existing empty object — insert into braces",
    source: `export default function T() { return <div data-dropin-id="eeeeeeee" style={{}}>x</div>; }`,
    op: { oid: "eeeeeeee", width: "248px" },
    expectChange: true,
  },
  {
    name: "missing oid — bail with reason",
    source: `export default function T() { return <div data-dropin-id="aaaaaaaa">x</div>; }`,
    op: { oid: "ffffffff", width: "248px" },
    expectChange: false,
    expectReason: /not found/,
  },
  {
    name: "self-closing tag with no attrs",
    source: `export default function T() { return <img data-dropin-id="gggggggg" src="/x.png" />; }`,
    op: { oid: "gggggggg", width: "300px", height: "200px" },
    expectChange: true,
  },
  {
    name: "no width or height (intentional no-op)",
...
for (const c of cases) {
  const r = applyResize(c.source, c.op);
  ...
  if (ok && c.expectReason === null && r.reason !== null) {
    ok = false;
  }
